...
- `themes/default.json` in the repo is the shipped seed: it is copied into
  `~/.config/hlit/themes/` at every launch, overwriting whatever is there.
  That's why the Default theme can't be overridden — the app also refuses
  to write to any shipped id.
